hexo.extend.helper.register('related_posts', function (currentPost, allPosts) {
  ...
  for (var i = 0; i < Math.min(relatedPosts.length, limit_num); i++) {
    ...
    result += '<div class="relatedPosts_item" style="background-color:' + hexo.theme.config.theme_color.main + '"><a class="relatedPosts_title" href="' + hexo.theme.config.rootConfig.root + relatedPosts[i].path + '" title="' + relatedPosts[i].title + '">';
    // result += '<img class="relatedPosts_cover '+ lazy_class + '"' + lazy_src + '="' + cover + '">';
    result += relatedPosts[i].title;
    result += '</a></div>'
    };
